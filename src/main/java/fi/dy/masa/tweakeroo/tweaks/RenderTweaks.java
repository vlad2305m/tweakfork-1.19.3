...
public class RenderTweaks
{
    ...
    public static void addSelectionToList() {
        ...
        int count = 0;
        ConcurrentHashMap<Long, ListMapEntry> list = (type == ListType.WHITELIST) ? SELECTIVE_WHITELIST : SELECTIVE_BLACKLIST;
    ...
    public static void renderSelection(MatrixStack matrices, Selection selection) {
        ...
        if (pos1 != null && pos2 != null)
        {
            ...
            {
                RenderUtils.renderBlockOutlineOverlapping(pos1, expand, lineWidthBlockBox, colorPos1, colorPos2, colorOverlapping, matrices, mc);
            }
        }
        else
        {
	...
	public static void onPistonEvent(BlockState state, World world, BlockPos pos, int type, int data) {
        ...
        if (!Configs.Generic.SELECTIVE_BLOCKS_TRACK_PISTONS.getBooleanValue() || !FeatureToggle.TWEAK_SELECTIVE_BLOCKS.getBooleanValue())
            ...
            if (type != 0 && !((MixinPistonBlock) state.getBlock()).getSticky()) return; // non sticky pistons do nothing
            ...
            boolean attatchedWhitelist = SELECTIVE_WHITELIST.containsKey(pos.offset(pushDirection, (type == 0) ? 1 : 2).asLong());
            boolean attatchedBlacklist = SELECTIVE_BLACKLIST.containsKey(pos.offset(pushDirection, (type == 0) ? 1 : 2).asLong());
            ...
            if (type == 0) { // extending
                if (attatchedWhitelist) {
                    ...
                    SELECTIVE_WHITELIST.put(pos.offset(pushDirection, 1).asLong(), new ListMapEntry(pos.offset(pushDirection, 1)));
                }
                if (attatchedBlacklist) {
                    ...
                    SELECTIVE_BLACKLIST.put(pos.offset(pushDirection, 1).asLong(), new ListMapEntry(pos.offset(pushDirection, 1)));
                }
            }
            if (moveSuccess)
			{
    ...
    public static String getStringFromMap(ConcurrentHashMap<Long, ListMapEntry> map) {
        ...
        while (iterator.hasNext()) {
            ...
            entries.add(entry.currentPosition.getX() + "," + entry.currentPosition.getY() + "," + entry.currentPosition.getZ());
        }
